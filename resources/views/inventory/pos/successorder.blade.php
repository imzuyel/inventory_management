                ...
                <div class="card">
                    ...
                    <div class="body">
                        <div class="table-responsive text-center">
                            <table
                                class="table table-bordered table-striped table-hover @if (count($success)>0) dataTable js-exportable @endif">
                                ...
                                <tbody>
                                    @php($i=1)
                                    @foreach ($success as $all)
                                    <tr>
                                        <td>{{ $i++ }}</td>
                                        <td>{{ $all->name }}</td>
                                        <td>{{ $all->oder_date }}</td>
                                        <td>{{ $all->total_product }}</td>
                                        <td>{{ $all->id }}</td>
                                        <td>{{ $all->payment_status }}</td>
                                        <td>
                                            @if ($all->oder_status=='pendding')
                                            <h4><span class="label label-danger">Pendding</span></h4>
                                            @else
                                            <h4><span class="label label-success">Approve</span></h4>
                                            @endif
                                        </td>
                                        <td>
                                            <a href="{{ route('order.views',['id'=>$all->id]) }}"
